...
namespace ZWL.BLL
{
	...
	public class ERPUser
	{
        ...
        /// 更新一条数据
        /// </summary>
        public void Update_zt(int zt)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update ERPUser set ");
            strSql.Append("zt='"+@zt+"'");
            String[] ID = UserName.Split(':');//提取工号
            strSql.Append(" where username in ('" + ID[0] + "')");
            SqlParameter[] parameters = {			
					new SqlParameter("@username", SqlDbType.VarChar,50)};
            parameters[0].Value = UserName;
            DbHelperSQL.ExecuteSql(strSql.ToString(), parameters);
        ...
        /// <summary>
        /// 更新一条状态数据 姓名版
        ///    update [DBOA].[dbo].[ERPUser] set zt='0' where TrueName =  '方铿滨'
        /// </summary>
        public void UpdateForname_zt(int zt)
        {
            String[] ID = UserName.Split(',');//解析字符串，导出人名
            String str1 = ID[0].Replace("''","");
            String str2 = ID[0].Substring(0,3);

            StringBuilder strSql = new StringBuilder();
            strSql.Append("update ERPUser set ");
            strSql.Append("zt='" + @zt + "'");

            strSql.Append(" where TrueName = '" + str2 +"'");
            SqlParameter[] parameters = {			
					new SqlParameter("@username", SqlDbType.VarChar,50)};
            parameters[0].Value = UserName;
            DbHelperSQL.ExecuteSql(strSql.ToString(), parameters);
        ...
        {
            /* select c.id,c.serils,c.username,c.TrueName,c.jiaose,c.zt,repairdepartment,c.orderno,c.state,c.DispatchingDate,c.orderdate from ( 
             * select a.id,a.serils,a.username,a.TrueName,jiaose,a.zt,b.repairdepartment,b.orderno,b.state,b.DispatchingDate,b.orderdate FROM [dbo].[ERPUser] a left join 
             * (select repairdepartment,orderno,state,DispatchingDate,orderdate,headpeople as rm from [dbo].[ERPOperations] 
             * where state in('2','3') union all select repairdepartment,orderno,state,DispatchingDate,orderdate,AssistPeople1 as rm from [dbo].[ERPOperations] 
             * where state in ('2','3') and AssistPeople1!=''  union all select repairdepartment,orderno,state,DispatchingDate,orderdate,AssistPeople2 as rm from [dbo].[ERPOperations] 
             * where state in ('2','3') and AssistPeople2!='') b  on a.username=b.rm where a.department='临床工程信息部' and a.jiaose like '%维修%' )c*/


            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select c.id,c.serils,c.username,c.TrueName,c.jiaose,c.zt,repairdepartment,c.orderno,c.state,c.DispatchingDate,c.orderdate from ( ");
            strSql.Append("select a.id,a.serils,a.username,a.TrueName,jiaose,a.zt,b.repairdepartment,b.orderno,b.state,b.DispatchingDate,b.orderdate FROM [dbo].[ERPUser] a left join"+ 
                "(select repairdepartment,orderno,state,DispatchingDate,orderdate,headpeople as rm from [dbo].[ERPOperations] "+
                "where state in('2','3') union all select repairdepartment,orderno,state,DispatchingDate,orderdate,AssistPeople1 as rm from [dbo].[ERPOperations] "+
                "where state in ('2','3') and AssistPeople1!=''  union all select repairdepartment,orderno,state,DispatchingDate,orderdate,AssistPeople2 as rm from [dbo].[ERPOperations] "+
                "where state in ('2','3') and AssistPeople2!='') b  on b.rm like concat('%',a.username,'%') where a.department='临床工程信息部' and a.jiaose like '%维修%' )c");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
